...
export default function GenerateMVP() {
  ...
  useEffect(() => {
    ...
    const fetchStartupDetails = async () => {
      try {
        ...
        const mvpResponse = await fetch(`http://localhost:3001/api/ai/mvp/${id}`, {
          method: 'GET', // Changed to GET to fetch MVP data
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const handleGenerateMVP = async () => {
    ...
    try {
      setGenerating(true);
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:3001/api/ai/mvp/${startup.id}`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-6 border-b border-gray-200">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">MVP Generator</h1>
            ...
            <div className="bg-white shadow overflow-hidden sm:rounded-lg">
              ...
              <div className="border-t border-gray-200">
                <dl>
                  ...
                  <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                    <dt className="text-sm font-medium text-gray-500">Recommended Technologies</dt>
                    <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                      <ul className="border border-gray-200 rounded-md divide-y divide-gray-200">
                  ...
                  {mvpData.mockupUrl && (
                    <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                      <dt className="text-sm font-medium text-gray-500">Mockup</dt>
                      <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                        <a 
                          href={mvpData.mockupUrl} 
                          target="_blank" 
              ...
              <div className="px-4 py-5 sm:px-6">
                <div className="flex justify-between">
                  ...
                  >
                    {generating ? 'Generating...' : 'Generate New MVP'}
                  </button>
                </div>
              </div>
            </div>
